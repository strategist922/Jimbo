...
window.sharejs.extendDoc 'attach_ace', (editor, keepEditorContents) ->
  ...
  doc.updateCursors = ->
    return unless window.clients
    @markers ?= []
    for marker in @markers
      editor.session.removeMarker marker
    ranges = []
    myId = if window.clients then window.clients.indexOf(doc.connection.id)
    inserts = 0
    indexToId = {}
    for own sessionId, cursor of @cursors
      user_id = window.clients.indexOf(sessionId)
      continue if user_id == -1
      range = cursorToRange(editorDoc, cursor)
      continue unless range
      @markers.push = (editor.session.addMarker range, "foreign_selection ace_selection user_#{user_id}", "line")
      ranges.push range
    ...
    for cursorElement, i in cursorLayer.cursors
      cursorElement.className = "ace_cursor user_#{indexToId[i]}"
    doc.cursor

  @on "cursors", doc.updateCursors
